...
// GET /api/lessons — list all lessons (metadata only, no full data)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { id, title, description, data, created_by, scene_count, interactive_mode } = body as {
      id: string;
      title?: string;
      description?: string;
      data: unknown;
    ...
    await sql`
      INSERT INTO lessons (id, title, description, data, created_by, scene_count, interactive_mode)
      VALUES (
        ${id},
        ${title ?? 'Untitled'},
        ${description ?? null},
